...
export const TimlienthongEffect = (nodes, edges) => {
  useEffect(() => {
    let network = null;
    ...
    document.getElementById("run").addEventListener("click", () => {
      const nodesInput = document.getElementById("nodes").value;
      const edgesInput = document
        .getElementById("edges")
        .value.trim()
        .split("\n");
      const g = new Graph(parseInt(nodesInput));
      ...
      // resultDiv.innerHTML = "<h2>Các thành phần liên thông mạnh</h2>";
      // sccs.forEach((component) => {
      //     resultDiv.innerHTML += `<p>Thành phần ${count}: ${component.length === 1 ? component[0] : component.join(", ")}</p>`;
      //     count++; // Tăng biến đếm
      // });

      // // Thêm CSS trực tiếp qua JavaScript
      // resultDiv.style.margin = "10px";
      // resultDiv.style.padding = "20px";
      // resultDiv.style.border = "2px solid #7e7e7e";
      // resultDiv.style.borderRadius = "8px";
      // resultDiv.style.backgroundColor = "#f9f9f9";
      // resultDiv.style.width = "250px";
      // resultDiv.style.height = "fit-content";

      // Vẽ đồ thị cho tất cả các thành phần liên thông mạnh
      drawSCCGraph(sccs, edgesDataSet);
    });

    // const divNetwork = document.getElementById("mynetwork");
    // divNetwork.style.width = "auto";
